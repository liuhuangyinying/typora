...
默认情况下Tomcat可以支持的最大请求数是150，当超过这个并发数的时候，可能会导致响应延迟，连接丢失等问题。

> 实践数据 : tomcat可以支持2000并发,受限于带宽和回包大小,某公众号项目400-600并发就会出现响应延迟问题.
>
> 并发数 = 带宽 (KB/s)/ 回包大小(KB)
>
> 当然,并发数还取决于其他的软硬件指标.
...
一兆带宽 1Mb/s , Byte(字节) = 8bit(位)

1Mb/s = 1024  kb/s = 1024/8 kB/s = 128 KB/s

扣除12%信息头标识各种控制讯号,所以实际上的速度应该是: 128 * 88% =112KB/s

 按照经验值来说,大约70%的使用是稳定的网络,因此1兆带宽,假设回包大小为1KB,那么可以支持的并发数应当是:

128 * 0.7 / 1 = 90 ;

但是某公众号项目,1兆带宽全部用完,可以支持400并发,可以计算出数据包大小:

128 * 1024 / 400  = 328B.

一个中文2B,大概可以支持164个中文字的数据包.
...
> 1. 线程数量不是越多越好,线程本身的创建销毁也耗费资源;
>
>    nginx和tomcat都可以配置线程
>
> 2. 不同操作系统对于进程中的线程数有一定限制:
>
>    比如windows每个进程中的线程数不允许超过2000;
>
>    比如linux每个进程中线程数不允许超过1000.(可以配置)
>
>    cat /proc/sys/vm/max_map_count
>
> 3. 带宽和数据包大小;
>
> 4. 就算带宽支持并发数很多,吞吐量上去了,CPU处理不过来,那么提高吞吐量就没有意义了;
>
>    某项目通过消息中间件提升吞吐量(异步处理),CPU消费能力弱于生产者,那么依然没有意义,
>
>    同时堆积的请求处理队列可能会耗费非常大的内存.
>
> 5. 内存
>
> 6. 网络通信机制
>
>    比如tomcat,tomcat7以下默认BIO,tomcat8及以上默认NIO
...
- ExecutorService 接口 线程池
  ...
  > 不推荐使用Executors创建线程池,创建线程池需要对各个参数了然于心,避免出现各种安全隐患问题:
>
  > 比如newCachedThreadPool线程池数量无限大Integer.MAX_VALUE
  >
  > 比如newFixedThreadPool堆积的请求处理队列可能会耗费非常大的内存
...
![锁升级.png](../typora/多线程笔记.assets/锁升级.png)
...
因为十六进制的2位正好是1字节，这里传输4字节,所以选十六进制0x01000000为例，如图所示，对小端存储，低位是0x01，应存入低位地址，所以存入的顺序是
...
注意该方法要捕获异常 ,比如有两个线程同时执行(没有Synchronized)，一个线程优先级为MAX_PRIORITY，另一 个为MIN_PRIORITY，如果没有Sleep()方法，只有高优先级的线程执行完成后，低优先级的线程才能执行；但当高优先级的线程sleep(5000)后，低优先级就有机会执行了。  总之，sleep()可以使低优先级的线程得到执行的机会，当然也可以让同优先级、高优先级的线程有执行的机会。
...
![ThreadLocal和Thread的类关系图](.\多线程笔记.assets\ThreadLocal和Thread的类关系图.png)
...
public void set(T value) {
    ...
    ThreadLocalMap map = getMap(t);		// 1 获取当前线程的成员变量ThreadLocalMap threadLocals
    if (map != null)
        map.set(this, value);			// 3 赋值
    else
        createMap(t, value);            // 2 初始化当前线程的成员变量ThreadLocalMap threadLocals
}
```

1 :: 获取当前线程的成员变量ThreadLocalMap threadLocals
...
2 :: 初始化当前线程的成员变量ThreadLocalMap threadLocals
...
![](..\typora\多线程笔记.assets\ReentrantLock原理.png)
...
![image-20201223184904588](F:\typora\typora\多线程笔记.assets\ReentrantLock核心类关系图.png)

```java
static Lock lock = new ReentrantLock();
```

> ReentrantLock :: lock 抢占锁
...
```java
final void lock() {
    // ##1 获得锁,state=1,exclusiveOwnerThread=currentThread
...
> AbstractOwnableSynchronizer :: 设置获得锁线程
...
> AbstractQueuedSynchronizer :: 锁状态
...
> 三步骤:
>
> 1. tryAcquire(arg) :: CAS尝试获得锁或者重入锁,成功后继续正常业务流程
>
> 2. addWaiter(Node.EXCLUSIVE) :: 加入AQS同步等待队列(双向链表) , 如果不存在则构建
>
> 3. acquireQueued :: 根据情况park线程,挂起线程将会在这里阻塞,直到有线程释放锁之后,唤醒队列.
>
>    AQS同步等待队列第一个节点CAS尝试获得锁,其他节点判断节点状态以及线程是否被中断,
...
>    就是CAS尝试争抢锁以及支持重入锁
>
>    返回true就表示得到锁,可以进入正常业务逻辑

tryAcquire(arg)  本质上调用Sync.nonfairTryAcquire(acquires)

```java
// ## AbstractQueuedSynchronizer 
// 由子类实现,这里默认是NonfairSync.tryAcquire(int acquires)
// 而这里默认是NonfairSync方法调用Sync.nonfairTryAcquire(acquires)
...
mode是互斥节点: static final Node EXCLUSIVE = null;  [waitStatus=0,thread=null]
...
private Node addWaiter(Node mode) {
    // 1.当前线程Node节点 :: [waitStatus=0 , thread = curreentThread]
    Node node = new Node(Thread.currentThread(), mode);
    // 2.等待队列是双向链表
    Node pred = tail;
    // 4.当AQS同步等待队列存在时,直接加入队列尾部
    if (pred != null) {
        node.prev = pred;
    ...
    // 3.首次进入,tail一定为null :: 构造等待队列双向列表: head -> new Node() -> node -> tail
    enq(node);
    return node;
}
...
private Node enq(final Node node) {
    for (;;) {
        Node t = tail;						   // 此时tail首次一定是空节点,需要初始化同步等待链表
        if (t == null) { // Must initialize
            if (compareAndSetHead(new Node())) // 1- 设置头结点 new Node() [waitStatus=0 , thread = null]
                tail = head;			       // 2- 设置尾节点 new Node()
        } else {
            node.prev = t;					   // 3- 线程节点node的向前指针指向new Node()
...
>    AQS同步等待队列第一个节点CAS尝试获得锁,其他节点判断节点状态以及线程是否被中断
>
>    这一步,所有的线程Node的前一个节点的waitStatus都是-1.
>
>    被阻塞的线程,一旦有线程释放锁,那么将从这里被唤醒,然后继续争抢锁.
...
final boolean acquireQueued(final Node node, int arg) {
    ...
    try {
        ...
        for (;;) {
            // 等待队列 head -> new Node() -> node -> tail ,这里的node是当前线程Node,p是上一个节点
            final Node p = node.predecessor(); 
            // ##1 p == head 表示当前线程Node是头节点,可以再次尝试CAS获得锁
            if (p == head && tryAcquire(arg)) {
                // 获得锁之后,将原来的头节点删除,线程节点[thread=>null]
                setHead(node);
                p.next = null; // help GC :: 删除new Node()
                failed = false;
                return interrupted;
            }
            // ##2 shouldParkAfterFailedAcquire 将头节点[waitStatus::0 => -1]设置为SIGNAL,表示可以被唤醒的线程
            // ##3 如果被唤醒parkAndCheckInterrupt()返回false,那么将开始CAS尝试获得锁
...
// 头节点指向当前节点,并且删除prev节点指向,thread设置为null
private void setHead(Node node) {
    head = node;
    node.thread = null;
    node.prev = null;
}
// AQS线程的前一个节点状态设置为SIGNAL,表示可以被唤醒的线程
...
> AbstractQueuedSynchronizer :: unlock 释放锁
>
> 释放锁过程:
>
> 1. 锁标识state-1,全部锁释放之后,exclusiveOwnerThread设置为null
> 2. 所有锁释放之后,如果AQS存在待唤醒线程,那么唤醒线程[头结点指向下一个节点]
...
private final void transfer(Node<K,V>[] tab, Node<K,V>[] nextTab) {
    ...
    if (nextTab == null) {            // initiating
        try {
            @SuppressWarnings("unchecked")
            Node<K,V>[] nt = (Node<K,V>[])new Node<?,?>[n << 1];// 扩容长度*2
            nextTab = nt;
        } catch (Throwable ex) {      // try to cope with OOME
            sizeCtl = Integer.MAX_VALUE;
            return;
        }
        nextTable = nextTab;// 扩容后的数组
        transferIndex = n;	// 扩容时数据转移的索引值
    }
...
线程的创建会消耗内存资源,增加CPU的开销(线程多的时候会有频繁上下文切换)

## 线程池的好处

- 复用线程:避免频繁创建销毁线程带来的性能开销
- 控制线程数:合理设置线程大小避免出现资源瓶颈
...
只有一个核心的线程池,FinalizableDelegatedExecutorService委派模式
...
![线程池设计.png](./多线程笔记.assets/线程池设计.png)
...
![线程池实现.png](./多线程笔记.assets/线程池实现.png)

### addWorker

1. 构建一个线程,并且启动                                                               
2. 创建线程之前,需要判断是否超过设定阈值;
3. [统计当前的工作线程数量] - > 考虑线程安全性(OK)
4. 存储的容器(池子)[HashSet<Worker>]
5. 线程的回收(复用或者销毁) getTask

**1- 构建一个线程,并且启动**

addWorker 中: 下面这四行等价于 => new Thread(new Worker(fisrtTask)).start();
...
Worker(Runnable firstTask) {
    ...
    // 这里getThreadFactory().newThread(this)等价于new Thread(new Worker(fisrtTask));
    this.thread = getThreadFactory().newThread(this);
}
```

因此,构造一个线程不是Worker也不会firstTask,而是另外一个线程:new Thread(new Worker(fisrtTask)).start();
...
**addWorke** :: ThreadPoolExecutor

```java
private boolean addWorker(Runnable firstTask, boolean core) {
    // 1- 判断线程数::并发场景,避免超出线程数,超出直接返回
    retry:
    for (;;) {
        int c = ctl.get();
        int rs = runStateOf(c);
    ...
    // Worker是一个线程  Worker w = new Worker(firstTask);
    Worker w = null;
    try {
        w = new Worker(firstTask);
        final Thread t = w.thread;
...
private Runnable getTask() {
    ...
    for (;;) {
        ...
        // timed :: wc > corePoolSize 总共工作线程数>核心线程数,才需要回收
        // 
...
CPU密集型:运算速率很高,依赖CPU  保持和CPU核心数量一致,8CORE,8个线程
